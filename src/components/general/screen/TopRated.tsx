...
import { getMoviesTopRated } from "../../../api/getMoviesTopRated";
...
export const TopRated = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const result = async () => {
      const movies: any = await getMoviesTopRated(page);
      setTimeout(() => {
        setLoading(false);
      }, 1000);
  ...
  return (
    <Grid>
      {loading ? (
        ...
        >
          {moviesTopRated.map((mov: any, id: number) => (
            <MovieCard
              key={id}
              movie={{
                image: {
