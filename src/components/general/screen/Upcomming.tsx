...
import { getMoviesUpcoming } from "../../../api/getMoviesUpcoming";
...
export const Upcomming = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const result = async () => {
      const movies: any = await getMoviesUpcoming(page);
      setTimeout(() => {
        setLoading(false);
      }, 1000);
  ...
  return (
    <Grid>
      {loading ? (
        ...
        >
          {moviesUpcomming.map((mov: any, id: number) => (
            <MovieCard
              key={id}
              movie={{
                image: {
